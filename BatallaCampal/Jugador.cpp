...
unsigned int Jugador::buscarIdSoldado(int fila, int columna, int altura) {
	if (fila < 1 || columna < 1) {
	...
	while (this->soldados->avanzarCursor() && (!encontrado)) {
		...
		if (this->soldados->getCursor()->getPosicionX() == fila
				...
				&& this->soldados->getCursor()->getPosicionZ() == altura) {
			encontrado = true;
...
int Jugador::buscarIdHerramienta(int fila, int columna, int altura) {
	if (fila < 1 || columna < 1 || altura < 1) {
		throw "No se puede buscar un armamento en una fila o columna que no tenga valor al menos 1.";
	}
	...
	while (this->herramientas->avanzarCursor() && (!herramientaEncontrada)) {
		...
		if (this->herramientas->getCursor()->getPosicionX() == fila
				...
				&& this->herramientas->getCursor()->getPosicionZ() == altura) {
...
bool Jugador::buscarSoldado(int fila, int columna, int altura) {
	bool encontrado = false;
	this->soldados->reiniciarCursor();
	while (this->soldados->avanzarCursor() && (!encontrado)) {
		if (this->soldados->getCursor()->getPosicionX() == fila
...
bool Jugador::buscarHerramienta(int fila, int columna, int altura) {
	bool herramientaEncontrada = false;
	this->herramientas->reiniciarCursor();
	while (this->herramientas->avanzarCursor() && (!herramientaEncontrada)) {
		if (this->herramientas->getCursor()->getPosicionX() == fila
...
void Jugador::eliminarSoldado(int fila, int columna, int altura) {
	if (fila < 1 || columna < 1 || altura < 1) {
		throw "No se puede eliminar un soldado en una posición inválida. La fila y la columna deben ser mayor o igual a 1.";
	}

	this->soldados->remover(this->buscarIdSoldado(fila, columna, altura));
...
void Jugador::eliminarHerramienta(int fila, int columna, int altura) {
	if (fila < 1 || columna < 1 || altura < 1) {
		throw "No se puede eliminar un armamento en una posición inválida. La fila y la columna deben ser mayor o igual a 1.";
	}

	this->herramientas->remover(this->buscarIdHerramienta(fila, columna, altura));
	this->cantidadHerramientas--;
}
